...
{
    public abstract class CrudWithAuditServiceBase<TEntity, TKey, TPost, TPut, TGet> : ReadonlyServiceBase<TEntity, TKey, TGet>, ICrudService<TKey, TPost, TPut, TGet>
        where TEntity : IEntity<TKey>, IUtcAuditableEntity
    {
        protected CrudWithAuditServiceBase(IUnitOfWork uow, IMapper mapper)
            : base(uow, mapper)
        {
        }
        ...
        public virtual async Task<TGet> UpdateAsync(TKey id, TPut data)
        {
            ...
            existing.ChangedAtUtc = DateTime.UtcNow;
